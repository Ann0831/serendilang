...
import { getRequestedFriendData, getUserLanguage, getProfilePictureUrl } from "../api/api.client.js";
...
export async function getAllFriendRequests() {
  try {
    ...
    handleServiceNetworkError(err, "getFriendRequestsData.js");
...
export async function enrichFriendRequestCardData(req) {
  ...
  try {
    ...
    if (langSettled.status === "rejected") handleServiceNetworkError(langSettled.reason, "getFriendRequestsData.js");
    if (picSettled.status === "rejected") handleServiceNetworkError(picSettled.reason, "getFriendRequestsData.js");
    ...
    const langSource =
      langRes && typeof langRes === "object"
        ? (langRes?.data && typeof langRes.data === "object" ? langRes.data : langRes)
        : {};
    const language = {
      nativelanguage:
        langSource?.nativelanguage ??
        langSource?.native_language ??
        langSource?.nativeLanguage ??
        req?.language?.nativelanguage ??
        "?",
      targetlanguage:
        langSource?.targetlanguage ??
        langSource?.target_language ??
        langSource?.targetLanguage ??
        req?.language?.targetlanguage ??
        "?",
    };
    const profilePicUrl =
