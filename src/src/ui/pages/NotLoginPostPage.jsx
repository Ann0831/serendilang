...
export default function NotLoginPostPage() {
  const s = useSubscribedState("NotLoginPostPage", {});
  ...
  const emitLoadMore = (from, showEndReachedNotice = false, isPreload = false) => {
    const now = Date.now();
    if (now - lastEmitAtRef.current < 450) return;
    lastEmitAtRef.current = now;
    eventBus.emit("notLoginPostPageLoadMore", {
      isPreload: !!isPreload,
      from: `${from}-bottom`,
  ...
  const checkNearBottom = (from) => {
    ...
    if (nearBottomInList) {
      emitLoadMore(from, true, false);
    }
  };

  const onListScroll = (e) => {
    if (!e?.currentTarget) return;
    checkNearBottom("ui/notLoginPost/list-scroll");
  };
  ...
  useEffect(() => {
    const onWindowScroll = () => {
      ...
      if (nearBottomInWindow) {
        emitLoadMore("ui/notLoginPost/window-scroll", true, false);
      } else if (inPreloadWindow) {
        emitLoadMore("ui/notLoginPost/window-preload", false, true);
      }
    };
  ...
  return (
    <div className="flex h-full min-h-full flex-col">
      <div ref={listRef} id="postpage-posts" onScroll={onListScroll} className="flex-1 flex flex-col gap-4 p-4 overflow-y-auto">
        {showEmpty ? <Empty text="No posts" /> : null}
        {showSkeleton ? [0, 1, 2].map((n) => (
          <article key={`not-login-post-skeleton-${n}`} className="w-[70%] min-w-0 mx-auto rounded-2xl shadow p-4 bg-white mb-6 border border-gray-200">
